...
type StatusMsgTestSuite struct {
	suite.Suite
	cfg              config.Config
...
`
	...
	c.Insert(bson.M{
		...
		"prev_status":         "OK",
		"prev_timestamp":      "2015-04-30T23:59:00Z",
		"summary":             "srv.typeA status is ok",
	...
	c.Insert(bson.M{
		...
		"prev_status":         "OK",
		"prev_timestamp":      "2015-05-01T00:00:00Z",
		"summary":             "srv.typeA status is critical",
...
func (suite *StatusMsgTestSuite) TestReadStatusDetail() {
   ...
   <report name="ROC_CRITICAL">
	...
	fullurl1 := "/api/v1/status/metrics/msg/cream01.afroditi.gr/" +
		"CREAM-CE/emi.cream.CREAMCE-JobSubmit?exec_time=2015-05-01T01:00:00Z&job=ROC_CRITICAL"

	fullurl2 := "/api/v1/status/metrics/msg/host01.eudat.gr/" +
		"srv.typeA/typeA.metric.Memory?exec_time=2015-05-01T01:00:00Z&job=EUDAT_CRITICAL"
...
func (suite *StatusMsgTestSuite) TearDownTest() {

	session, _ := mongo.OpenSession(suite.cfg.MongoDB)

	session.DB("argotest_msg").DropDatabase()
	session.DB("argotest_msg_eudat").DropDatabase()
	session.DB("argotest_msg_egi").DropDatabase()
}
